...
export function initializeASCII() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.fonts.ready.then(() => {
      ...
      const charPositions = ascii_split.map((char, index) => {
        ...
        const x =
          itemRect.left + itemRect.width / 2 + window.scrollX - containerX;
        const y =
          itemRect.top + itemRect.height / 2 + window.scrollY - containerY;
        return {
          char: char,
          x: x,
          y: y,
      ...
      const processActiveArea = (pointX: number, pointY: number) => {
        ...
        charactersToProcess.forEach(({ char, x: charX, y: charY, index }) => {
          ...
          const distance = Math.max(
            0,
            Math.hypot(pointX - charX, pointY - charY)
          );
          ...
          if (distance >= MAX_DISTANCE) {
            ...
            const fontWeight =
              MIN_FONT_WEIGHT +
              weightRatio * (MAX_FONT_WEIGHT - MIN_FONT_WEIGHT);
